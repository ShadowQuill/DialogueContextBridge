...
| 文件 | 作用 |
| --- | --- |
| `index.ts` | 统一注册全部 **17 个命令**（`/compile`、`/save`、`/dcb-save`、`/snapshot-search`、`/snapshot.list`、`/snapshot.show`、`/snapshot.remove`、`/snapshot.history`、`/snapshot.rollback`、`/snapshot-weight`、`/import`、`/dcb-merge`、`/dcb-import-last`、`/dcb-merge-last`、`/dcb`、`/dcb-export`、`/dcb-import`）。命令分两类：**带参命令**（`/import <id>`、`/dcb-merge <id>` 等）声明 `input` 提示，手打带参命令会被宿主直接拦截执行、不再误发模型（根治历史 `/import ... --mode merge` 烧 token 问题）；**无参免手打命令**（`/dcb-import-last`、`/dcb-merge-last`）不声明 `input`，由命令面板点选即执行、零手打、零气泡（DSH 0.1.x rc.2 手打带参命令才会出现「执行中…」常驻气泡，无参命令无此现象） |
...
| `manage.ts` | `/snapshot.list`、`/snapshot.show`、`/snapshot.remove`、`/snapshot.history`、`/snapshot.rollback`、`/snapshot-weight`（查看 / 设置 / 清零快照合并权重，供 `--policy weighted` 使用） |
| `import.ts` | `/import`、`/dcb-merge`、`/dcb-import-last`、`/dcb-merge-last` 与 `/dcb`：把历史快照注入当前对话。其中 `/dcb-merge <id>` 是 `/import <id> --mode merge` 的**高频单步别名**（少一次交互），支持 `--policy newWins\|snapshotWins\|timestamp\|weighted` 与 `--dry-run`；`/dcb` 不带 id 时显示记忆库总览与快捷操作；`/import` 不带 id 时显示最近 N 条快照选择器（绕开手打 id 的气泡摩擦）；`/dcb-import-last` / `/dcb-merge-last` 为**无参免手打命令**，命令面板点选即可引入 / 融合最近一条，零手打、零气泡。全部支持 `inject`/`merge` 双模式与冲突裁决 |

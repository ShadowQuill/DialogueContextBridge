...
| 文件 | 作用 |
| --- | --- |
| `index.ts` | 统一注册全部 **15 个命令**（`/compile`、`/save`、`/dcb-save`、`/snapshot-search`、`/snapshot.list`、`/snapshot.show`、`/snapshot.remove`、`/snapshot.history`、`/snapshot.rollback`、`/snapshot-weight`、`/import`、`/dcb-merge`、`/dcb`、`/dcb-export`、`/dcb-import`）。**每个命令都声明 `input` 输入提示**，因此手打带参命令也会被宿主直接拦截执行、不再误发模型（根治历史 `/import ... --mode merge` 烧 token 问题），菜单点选与手打行为一致 |
...
| `import.ts` | `/import`、`/dcb-merge` 与 `/dcb`：把历史快照注入当前对话。其中 `/dcb-merge <id>` 是 `/import <id> --mode merge` 的**高频单步别名**（少一次交互），支持 `--policy newWins\|snapshotWins\|timestamp\|weighted` 与 `--dry-run`；`/dcb` 不带 id 时显示记忆库总览与快捷操作。全部支持 `inject`/`merge` 双模式与冲突裁决 |
